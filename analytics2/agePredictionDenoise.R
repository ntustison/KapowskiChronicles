...
#       predictedAge <- predict( brainAgeLM, testingData )
  ...
  rmsePlot <- ggplot( resultsData, aes( x = RMSE, fill = Pipeline ) ) +
                      ...
                      scale_x_continuous( "RMSE", limits = c( 7, 10 ) ) +
                     geom_density( alpha = 0.5 )
  ggsave( filename = paste( "~/Desktop/rfRmseDenoise", p, ".pdf", sep = "" ), plot = rmsePlot, width = 6, height = 6, units = 'in' )
  cat( "Mean ANTsDenoise rmse = ", mean( resultsData$RMSE[which( resultsData$Pipeline == 'ANTsDenoise' )], na.rm = TRUE ), "\n", sep = '' );
