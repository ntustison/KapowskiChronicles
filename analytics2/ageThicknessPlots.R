...
resultsIXI <- read.csv( 'labelresultsAntsAtithI.csv' )
resultsKirby <- read.csv( 'labelresultsAntsAtithK.csv' )
resultsNKI <- read.csv( 'labelresultsAntsAtithN.csv' )
resultsOasis <- read.csv( 'labelresultsAntsAtithO.csv' )
...
#                scale_y_continuous( "Thickness (mm)", breaks = seq( -1, 1, by = 1 ), labels = seq( -1, 1, by = 1 ), limits = c( -1, 1 ) ) +
  ...
  ggsave( filename = paste0( "AntsAtithResults/", corticalLabels[i], "_results.pdf" ), plot = thickPlot, width = 8, height = 6, units = 'in' )
  }
