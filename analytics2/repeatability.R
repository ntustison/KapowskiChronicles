...
kirby <- read.csv( "labelresultsPairwiseFreeSurferK.csv" )
...
iccData <- data.frame( First = as.vector( data.matrix( repeatabilityDataFrame[which( repeatabilityDataFrame$ScanOrder == "First" ), indices] ) ),
                       Repeat = as.vector( data.matrix( repeatabilityDataFrame[which( repeatabilityDataFrame$ScanOrder == "Repeat" ), indices] ) ) )
...
ggsave( filename = paste( "repeatabilityFreeSurferBA.pdf", sep = "" ), plot = repeatabilityPlot, width = 8, height = 6, units = 'in' )
...
repeatabilityError <- 100 * abs( repeatabilityDataFrame[which( repeatabilityDataFrame$ScanOrder == "First" ), indices] -
  ...
  ( 0.5 * (  repeatabilityDataFrame[which( repeatabilityDataFrame$ScanOrder == "First" ), indices] +
    repeatabilityDataFrame[which( repeatabilityDataFrame$ScanOrder == "Repeat" ), indices] ) )
repeatabilityError2 <- abs( repeatabilityDataFrame[which( repeatabilityDataFrame$ScanOrder == "First" ), indices] -
  repeatabilityDataFrame[which( repeatabilityDataFrame$ScanOrder == "Repeat" ), indices] )
...
for( i in seq( 1, nrow( repeatabilityDataFrame ), by = 2 ) )
  ...
  if( i == 1 )
    {
    repDataFrame <- repeatabilityDataFrame[i,1:endIndex]
    repDataFrame[1,indices] <- repeatabilityDataFrame[i,indices] - repeatabilityDataFrame[i+1,indices]
    }
  else
    {
    tmp <- repeatabilityDataFrame[i,1:endIndex]
    tmp[1,indices] <- repeatabilityDataFrame[i,indices] - repeatabilityDataFrame[i+1,indices]
    repDataFrame <- rbind( repDataFrame, tmp )
    }
  }
repDataFrame$AVERAGE_DIFF <- rowMeans( repDataFrame[,indices] )

summary( lm( AVERAGE_DIFF ~ 1 + SITE + SEX + AGE + VOLUME, data = repDataFrame ) )

for( i in numberOfLabels )
  {
  myformula <- as.formula( paste0( corticalLabels[i], "~ 1 + SITE + SEX + AGE + VOLUME" ) )
