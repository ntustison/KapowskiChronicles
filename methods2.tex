...
each subject has T1, T2, and FA images which are all aligned in the same subject space, 
...
An optimal template \cite{avants2010}, i.e., a mean shape and intensity image 
...
Brain extraction and segmentation results from the ANTs pipeline were visually 
inspected for failures and ``reasonableness'' for the 1200 images.  This cursory
quality control step consisted of a script which displayed the three canonical views for all
processed images in a time-series of ITK-SNAP%
\footnote{
http://www.itksnap.org
}
window displays.  A similar randomly sampled inspection was performed for the FreeSurfer 
volumetric segmentation results.  No manual changes were made for any component of either
pipeline.
